...
class SwingController:
    ...
    def next_foot_location(self, swing_prop, leg_index, state, command):
        ...
        touchdown_location = self.raibert_touchdown_location(leg_index, command)  # noqa

        time_left = self.config.dt * self.config.swing_ticks * (1.0 - swing_prop)   # noqa

        v = (touchdown_location - foot_location) / time_left * np.array([1, 1, 0])   # noqa

        delta_foot_location = v * self.config.dt

        z_vector = np.array([0, 0, swing_height_ + command.height])

        return foot_location * np.array([1, 1, 0]) + z_vector + delta_foot_location  # noqa
