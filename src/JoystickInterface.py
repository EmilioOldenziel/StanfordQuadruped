...
class JoystickInterface:
    ...
    def get_command(self, state, do_print=False):
        try:
            ...
            # Check if requesting a state transition to trotting, or from
            # trotting to resting
            gait_toggle = msg["R1"]
            command.trot_event = (gait_toggle == 1 and self.previous_gait_toggle == 0)  # noqa

            # Check if requesting a state transition to hopping, from trotting
            # or resting
            hop_toggle = msg["x"]
            command.hop_event = (hop_toggle == 1 and self.previous_hop_toggle == 0)  # noqa

            activate_toggle = msg["L1"]
            command.activate_event = (activate_toggle == 1 and self.previous_activate_toggle == 0)  # noqa
